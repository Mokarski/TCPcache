...
int
signals_txt (char *packed_txt_string)
{
  int n = 0;
  int result = 0;
  char Value[3];
...
//char *istr3;
  ...
  strcpy(Signal_Array[n].TCP_Type, istr1);
...
int
sSerial_by_num (int n, char *pack_buf)
{				//for thread solution
//int n=0;
  int result = 0;
  char Value[3];
  ...
  if (strlen (Signal_Array[n].Name) > 3)	//if name signal more then 4 symbols
    {
      ...
      strcat (pack_buf, Signal_Array[n].TCP_Type);
      //strcat (pack_buf, Value);
      strcat (pack_buf, ":");
...
int
sSerial_by_num_short (int n, char *packed_txt_string)
{				//for thread solution
//int n=0;
  int result = 0;
  char Value[3];
...
int
signal_update_ex (char sName[100], int sVal, int Execution_State)
{				//function to update parameters in signals array
  int n = 0;
  int cnt = 0;
  for (n = 0; n < MAX_Signals; n++)
    {
...
void
print_signals_list (void)	//print all signals
{
  register int t;
  for (t = 0; t < MAX_Signals; t++)
    {
...
void
print_by_name (char sName[100])	//print all signals
{
  register int t;
  int res = 0;
  for (t = 0; t < MAX_Signals; t++)
  ...
  if (res == 0)
    printf ("Not found signal with this signal NAME\n\n");
}
...
int
signal_update (char sName[100], int sVal)
{				//function to update parameters in tcp_cache_server list
  int n = 0;
  int cnt = 0;
  for (n = 0; n < MAX_Signals; n++)
    {
...
int
text_parser (char *str_in, char str_out1[STR_LEN_TXT],
	     char str_out2[STR_LEN_TXT], char str_out3[STR_LEN_TXT],
	     char str_out4[STR_LEN_TXT], char str_out5[STR_LEN_TXT])
{
  int i = 0;
  i = strlen (str_in);
  int x = 0;
