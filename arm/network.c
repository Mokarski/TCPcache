...
int
Data_to_sName (char *tcp_buffer)
{				//extract from tcp buffer signal and put to Name field
  char sep[10] = ";";
...
int
tcpsignal_read (char *message_in)
{
...
frame_read_s (char *message_in, short int signals_count)
{
  ...
  if (strlen (message_in) > MAX_MESS)
    {
      printf
	("Exeeded maximum buffer size, message_size: %i > then buffer_size:%i",
	 strlen (message_in), MAX_MESS);
      return -1;
    }
  ...
  Frame_Container.len =
    strlen (Frame_Container.data) + strlen (Frame_Container.type);
  printf ("Frame[ type:%s len:%i count:%i data{ %s } ] \n\r",
	  Frame_Container.type, Frame_Container.len, Frame_Container.count,
	  Frame_Container.data);
...
frame_tcpreq (char *msg)
{
  ...
  if (strlen (server_reply) > 5)
    {				//if response from server more then 4 symbols, then we get signals
...
int
frame_pack (char *type, char *message_in, char *message_out)
{				//construct frame frome message_in and put result to message type is "rd" or "wr"
  //char message [MAX_MESS]; //max size for 1 packet send for tcp is 65534 bytes 
  char c_len[4];
  char c_count[4];
...
int
frame_unpack (char *server_reply, char *data)
{				// copy serialized signals into data and return 1 if read or 2 if write
  char sep[10] = "#";
  char type[5];
  char *istr;
  char header[100];
  ...
  if (istr != NULL)
    ...
    {
      printf ("data_extract: Header - NULL! \n\r");
      return -1;
    }
  ...
  if (istr != NULL)
    ...
    {
      printf ("data_extract: PACKET - NULL! \n\r");
      return -1;
    }
  ...
  if (istr != NULL)
    {				//type of frame read or write
      printf ("Type^{%s}\n\r", istr);
      strcpy (type, istr);
      if (strstr (type, "rd"))
	ret_rd_wr = 1;		//read request
      if (strstr (type, "wr"))
	ret_rd_wr = 2;		//write request
      if (strstr (type, "Ok!"))
	ret_rd_wr = 3;		//ack - ok
      if (strstr (type, "err"))
	ret_rd_wr = 4;		//server return "error request"
      if (strstr (type, "ret"))
	ret_rd_wr = 5;		//retry request, if packet len no OK
    }
  else
    {
      printf ("data_extract: Header1 - NULL! \n\r");
      return -1;
    }
  ...
  if (istr != NULL)
    ...
    {
      printf ("data_extract: Header2 - NULL! \n\r");
      return -1;
    }
...
int
tcpsignal_write (char *message_in, int iVal)
{
...
tcpsignal_packet_write (char *message_in)
{
