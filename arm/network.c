...
int get_packet_len(char *srvr_reply){
	char sep ='#';
	char dat[MAX_MESS]={0};
	char *type;
	char *istr, *start, *data;
	char *header;
	char *c_len;
	size_t len;

	if(!strcmp(srvr_reply, "Ok!")) {
		return 1;
	}

	header = srvr_reply;
	start = strchr(srvr_reply, ';');
	data = strchr(srvr_reply, '#');
	if(!start || !data) {
		return 0;//-1;
	}
...
int frame_tcpreq (char *msg){
	...
	//Receive a reply from the server
	int bytes = 0, buflen = 0;
	while ((bytes = recv(sock , tmp , MAX_MESS , 0)) > 0)
	{
		memcpy(&server_reply[buflen], tmp, bytes);
		buflen += bytes;
		server_reply[buflen] = 0;
		ret=1;
		if (get_packet_len(server_reply)) {
			break;
		}
	}
	/*else {
