...
}  Signal_Array [MAX_Signals];

Signal_Array Signals_Matrix;
//Signal_Array pSA = *Signal_Matrix;
struct Reg {
int Mb_Reg;
int Mb_Bit;
} Reg_Bit;
...
{
 register int t;
 for(t=0; t<MAX_Signals; ++t) Signals_Matrix[t].Name[0] = '\0';
}
...
if(Signals_Matrix[t].Name[0]) 
  {
   printf("Signal Name: %s\n", Signals_Matrix[t].Name);
   Signals_Matrix[t].MB_Id=atoi(Signals_Matrix[t].Reserv1);
   printf("MB ID: %i\n", Signals_Matrix[t].MB_Id);
   
   Signals_Matrix[t].MB_Reg_Num=atoi(Signals_Matrix[t].Reserv2);
   printf ("MB Register Num: %i\n", Signals_Matrix[t].MB_Reg_Num);
   Signals_Matrix[t].Bit_Pos=atoi(Signals_Matrix[t].Reserv3);
   printf ("MB Register BitNum: %i\n", Signals_Matrix[t].Bit_Pos);
   printf ("Value type: %s\n", Signals_Matrix[t].Val_Type);
//     printf ("Reserve1: %s\n", Signal_Array[t].Reserv1); //reserved for file input conversion
//     printf ("Reserve2: %s\n", Signal_Array[t].Reserv2);
//     printf ("Reserve3: %s\n", Signal_Array[t].Reserv3);
     
     printf ("Signal OFF: %i\n", Signals_Matrix[t].Off);
     printf ("Execution flow state: %i\n", Signals_Matrix[t].ExState);
     printf ("Hardware execution mark: %i\n", Signals_Matrix[t].Ex_HW);
     printf ("Software execution mark: %i\n", Signals_Matrix[t].Ex_SF);
     printf ("Priority mark: %i\n", Signals_Matrix[t].Prio);

   printf ("Values: [Hi: %i] [Low: %i]\n\n", Signals_Matrix[t].Value[0],Signals_Matrix[t].Value[1]);
  }
}
printf("\n\n");
}
...
int file_load(void )
{
 ...
 while (1)
 {
   ...
   if (row == NULL)
    ...
    text_parser(str,Signals_Matrix[c].Name, Signals_Matrix[c].Val_Type, Signals_Matrix[c].Reserv1, Signals_Matrix[c].Reserv2, Signals_Matrix[c].Reserv3);
...
int cnt=0;
  ...
  {
   if (strstr(Signals_Matrix[n].Name,sName) != NULL) 
      {
        Signals_Matrix[n].Value[1]=sVal;
        cnt++;
        break;
      } 
  }
...
int cnt=0;
  for (n=0;n<MAX_Signals;n++)
  {
   if (strstr(Signals_Matrix[n].Name,sName) != NULL) 
      {
        Reg_Bit.Mb_Reg=Signals_Matrix[n].MB_Reg_Num;
        Reg_Bit.Mb_Bit=Signals_Matrix[n].Bit_Pos;
        cnt++;
        break;
      } 
  }
...
int result=0;
  for (n=0;n<MAX_Signals;n++)
  {
   if (strstr(Signals_Matrix[n].Name,sName) != NULL) 
      {
        result=Signals_Matrix[n].Value[1];
        break;
      } 
  }
...
int result=0;
  for (n=0;n<MAX_Signals;n++)
  {
   if (Signals_Matrix[n].Value[1] > 0)
      {
//        strcpy(sName,Signal_Array[n].Name);
//        result=Signal_Array[n].Value[1];
        printf ("[sName: %s ] [Value: %i ]",Signals_Matrix[n].Name, Signals_Matrix[n].Value[1]);
           if (strstr(Signals_Matrix[n].Name,"kei") == NULL) // search only button key pressed event
              {
                rl_write(1);
              }
      } 
